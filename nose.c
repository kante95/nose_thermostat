...
double normrand(){
	return sqrt( ( -2*log(1-myrandom())))*cos(3.1415*2*myrandom());
} 
...
int simulation(double density,double temperature,double time, double step){
    ...
    for(t = 0; t<time;t+=step){
        ...
        for(i=0;i<N;i++){
			for(j=0;j<3;j++){
				positions[i][j] = positions[i][j] + velocities[i][j]*step + 0.5*(current_accelerations[i][j]-s*velocities[i][j])*pow(step,2);
		...
		for(i=0;i<N;i++){
			for(j=0;j<3;j++){
        		half_velocities[i][j] = velocities[i][j] + 0.5*(current_accelerations[i][j] -s*velocities[i][j])*step;
    		}
    	}
        ...
        for(i=0;i<N;i++){
			for(j=0;j<3;j++){
        		velocities[i][j] = (half_velocities[i][j]+0.5*step*new_accelerations[i][j])/(1+0.5*step*s);
    		}
    	}
        ...
        printf("Energy at second %lf: %lf s: %lf\n",t,energy,s);
    }
    return 0;
}
